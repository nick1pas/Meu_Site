...
if(isset($_SESSION["ICP_UserName"]) && !empty($_SESSION["ICP_UserName"])){
			...
			if(count($charSkillsClassArr) > 0){
				...
				for($x=0;$x<(count($charSkillsClassArr)-1);$x++){
					...
					if($tpl->exists("BLOCK_ICP_PANEL_SKILLS",true)){
						$charSkills = $getPanelInfo->showCharSkills($config["L2jVersaoRussa"] ? $charSkillsClassArr[$x] : $x,$_GET["char_id"] ?? "",$_SESSION["ICP_UserName"]);
			...
			if(count($itemBroker) > 0){
				foreach($itemBroker as $rank){
					...
					if($tpl->exists("PANEL_CHAR_BROKER_DETAILS"))
						$tpl->PANEL_CHAR_BROKER_DETAILS = "<div class='d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center m-1 border-bottom' style='color:#fff;'><h5>Information</h5></div><table><tr><td style='padding:5px 10px; text-align:center;' valign='middle'><img src='images/races/".$charDetails[16]."' style='border-radius:5px; border:1px solid #aaa; width:100px; height:100px;'><br>".$charDetails[15]."</td><td style='padding:5px 10px;' valign='middle'><font color='#b09979'>Base class:</font> ".$charDetails[0]."<br><font color='#b09979'>Subclass:</font> ".$charDetails[1]."<br><font color='#b09979'>Nobless:</font> ".$charDetails[2]."<br><font color='#b09979'>Hero:</font> ".$charDetails[3]."<br><font color='#b09979'>Karma:</font> ".$charDetails[4]."<br><font color='#b09979'>Level:</font> ".$charDetails[5]."<br><font color='#b09979'>Sex:</font> ".$charDetails[6]."</td><td style='padding:5px 10px;' valign='middle'><font color='#b09979'>Online time:</font> ".$charDetails[7]."<br><font color='#b09979'>Last Access:</font> ".$charDetails[8]."<br><font color='#b09979'>Clan:</font> ".$charDetails[9]."<br><font color='#b09979'>Ally:</font> ".$charDetails[10]."<br><font color='#b09979'>PvP's:</font> ".$charDetails[11]."<br><font color='#b09979'>Pk's:</font> ".$charDetails[12]."<br><font color='#b09979'>Location:</font> ".$charDetails[13]."</td></tr></table>";
			...
			if(count($charBroker) > 0){
				foreach($charBroker as $rank){
					...
					if($tpl->exists("PANEL_CHAR_BROKER_CLAN"))
						$tpl->PANEL_CHAR_BROKER_CLAN = $charDetails[9];
					if($tpl->exists("PANEL_CHAR_BROKER_ALLYANCE"))
						$tpl->PANEL_CHAR_BROKER_ALLYANCE = $charDetails[10];
					if($tpl->exists("PANEL_CHAR_BROKER_PVP"))
						$tpl->PANEL_CHAR_BROKER_PVP = $charDetails[11];
					if($tpl->exists("PANEL_CHAR_BROKER_PK"))
						$tpl->PANEL_CHAR_BROKER_PK = $charDetails[12];
					if($tpl->exists("PANEL_CHAR_BROKER_LOCATION"))
						$tpl->PANEL_CHAR_BROKER_LOCATION = $charDetails[13];
					if($tpl->exists("PANEL_CHAR_BROKER_TYPE")){
						if($rank["charType"] == 1)
							$saleType = "Normal";
						elseif($rank["charType"] == 2)
					...
					for($x=0;$x<count($model);$x++){
						if(in_array($model[$x],$allowedBlockItems)){
							if($tpl->exists($model[$x],true)){
								$itemShow = $getPanelInfo->showCharacterItems($model[$x] != $allowedBlockItems[0] ? $model[$x] == $allowedBlockItems[1] ? "INVENTORY" : "WAREHOUSE" : "PAPERDOLL",$charDetails[14],$rank["charAccount"]);
								if(count($itemShow) > 0){
					...
					if($tpl->exists("BLOCK_ICP_PANEL_CHAR_BROKER_SKILLS_CLASS",true)){
						$charSkillsClass = $getPanelInfo->showSubClasses($charDetails[14],$rank["charAccount"]);
						if(!empty($charSkillsClass)){
							$charSkillsClassArr = explode(";",$charSkillsClass);
							if(count($charSkillsClassArr) > 0){
								$btn = null;
								for($x=0;$x<(count($charSkillsClassArr)-1);$x++){
									...
									if($tpl->exists("BLOCK_ICP_PANEL_CHAR_BROKER_SKILLS",true)){
										$charSkills = $getPanelInfo->showCharSkills($config["L2jVersaoRussa"] ? $charSkillsClassArr[$x] : $x,$charDetails[14],$rank["charAccount"]);
										if(count($charSkills) > 0){
					...
					if($tpl->exists("BLOCK_ICP_PANEL_CHAR_BROKER_QUESTS",true)){
						$charQuests = $getPanelInfo->showCharQuests($charDetails[14],$rank["charAccount"]);
						if(count($charQuests) > 0){
							foreach($charQuests as $rank){
								if($tpl->exists("PANEL_QUEST_OWNER_NAME"))
									$tpl->PANEL_QUEST_OWNER_NAME = $rank["questOwner"];
	...
	if($tpl->exists("CONFIG_ICP_PANEL",true)){
		if(isset($_SESSION["ICP_UserAccessLevel"]) && $_SESSION["ICP_UserAccessLevel"] == 10){
			if($tpl->exists("CONFIG_ICP_PANEL_RUSS_BASE"))
				$tpl->CONFIG_ICP_PANEL_RUSS_BASE = $config["L2jVersaoRussa"] ? " checked" : null;
			if($tpl->exists("CONFIG_ICP_PANEL_ACIS_BASE"))
				$tpl->CONFIG_ICP_PANEL_ACIS_BASE = $config["L2jVersaoAcis"] ? " checked" : null;
			if($tpl->exists("CONFIG_ICP_PANEL_OLD_ACIS"))
				$tpl->CONFIG_ICP_PANEL_OLD_ACIS = $config["l2jOldAcis"] ? " checked" : null;
			...
			if($tpl->exists("CONFIG_ICP_PANEL_PC_TEMPLATES"))
				$tpl->CONFIG_ICP_PANEL_PC_TEMPLATES = $getPanelInfo->showDir("templates/desktop/",$config["PC_TEMPLATE"]);
			if($tpl->exists("CONFIG_ICP_PANEL_MB_TEMPLATES"))
				$tpl->CONFIG_ICP_PANEL_MB_TEMPLATES = $getPanelInfo->showDir("templates/mobile/",$config["MB_TEMPLATE"]);
			...
			if($tpl->exists("CONFIG_ICP_PANEL_CHRONICLE"))
				$tpl->CONFIG_ICP_PANEL_CHRONICLE = $getPanelInfo->chronicle($config["CHRONICLE"]);
